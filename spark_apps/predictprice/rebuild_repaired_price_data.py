...
def _affected_product_ids(
    conn,
    repair_date: str | None,
    repair_since: str | None,
) -> list[str]:
    if repair_since:
        where_clause = "r.created_at >= :repair_since"
        params = {"repair_since": repair_since}
    else:
        where_clause = "DATE(r.created_at) = :repair_date"
        params = {"repair_date": repair_date}
    rows = conn.execute(
        text(
            f"""
...
def _active_product_ids(conn, product_ids: list[str]) -> list[str]:
    query = text(
        """
    ...
    return sorted(
        str(value)
        for value in conn.execute(query, {"product_ids": product_ids}).scalars()
        if value
    )


def _delete_current_forecasts(conn, product_ids: list[str]) -> int:
    if not product_ids:
        return 0
    query = text(
        """
...
def main() -> int:
    ...
    parser.add_argument(
        "--repair-since",
        help="Only include repair log rows created at or after this MySQL datetime.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Keep historical forecasts and replace only today's snapshot for active products.",
    )
    ...
    with engine.connect() as conn:
        product_ids = _affected_product_ids(conn, args.repair_date, args.repair_since)
        if not product_ids:
            print("No repaired products found for the requested repair window.")
            return 0
    ...
    with engine.begin() as conn:
        ...
        if args.preserve_forecast_history:
            forecast_product_ids = _active_product_ids(conn, product_ids)
            deleted_forecasts = _delete_current_forecasts(conn, forecast_product_ids)
        else:
            forecast_product_ids = product_ids
    ...
    if args.preserve_forecast_history:
        print(f"Historical forecasts preserved; current snapshots removed: {deleted_forecasts}")
    else:
